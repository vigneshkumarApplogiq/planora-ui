...
export function ActivityView({ project, user }: ActivityViewProps) {
  ...
  const loadActivities = async () => {
    ...
    try {
      ...
      const response = await activityApiService.getActivities(projectId, params)
  ...
  const renderActivityDetails = (activity: Activity) => {
    ...
    if (activity.story_points) {
      return (
        <div className="mt-2 text-sm text-muted-foreground">
          Story points: {activity.story_points} • Entity ID: {activity.entity_id}
        </div>
      )
    }
    ...
    if (activity.planned_points) {
      return (
        <div className="mt-2 text-sm text-muted-foreground">
          Duration: {activity.duration_weeks} weeks • Planned points: {activity.planned_points}
        </div>
      )
    }
        ...
        <CardContent>
          {loading ? (
            ...
            <div className="text-center py-12">
              ...
              <p className="text-muted-foreground">There are no activities matching your filters</p>
            </div>
          ) : (
            <>
              <div className="space-y-4">
                {activities.map((activity) => (
                  <div
                    key={activity.id}
                    className={`flex items-start space-x-4 p-4 border-l-4 ${getActivityColor(activity.activity_type)} bg-gray-50 dark:bg-gray-800/50 rounded-r-lg`}
                  >
                    <Avatar className="w-8 h-8 mt-1">
                      <AvatarFallback className="bg-[#28A745] text-white text-xs">
                        {activity.user_name?.split(' ').map(n => n[0]).join('').toUpperCase() || 'U'}
                      </AvatarFallback>
                    </Avatar>
              ...
              <div className="flex items-center justify-between mt-6 pt-4 border-t">
                <div className="text-sm text-muted-foreground">
                  Page {currentPage} • Showing {activities.length} of {totalCount} activities
                </div>
                <div className="flex items-center gap-2">
                  <Button
                    variant="outline"
