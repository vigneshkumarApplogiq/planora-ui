...
export function ReportsView({ project, user }: ReportsViewProps) {
  ...
  const [cycleTimeData, setCycleTimeData] = useState<CycleTimeMetrics | null>(
    null
  );
  ...
  useEffect(() => {
    const loadReportData = async () => {
      ...
      try {
        ...
        switch (selectedTab) {
          case "overview":
            const overview = await kanbanReportsApiService.getOverviewMetrics(
              project.id,
              dateParams
            );
          ...
          case "flow":
            const flow = await kanbanReportsApiService.getFlowMetrics(
              project.id,
              dateParams
            );
            setFlowData(flow);
          ...
          case "cycle":
            const cycleTime = await kanbanReportsApiService.getCycleTimeMetrics(
              project.id,
              dateParams
            );
          ...
          case "wip":
            const wip = await kanbanReportsApiService.getWIPAnalysis(
              project.id,
              { ...dateParams, aging_threshold_days: 7 }
            );
            setWipData(wip);
            break;

          case "team":
            const team = await kanbanReportsApiService.getTeamPerformance(
              project.id,
              dateParams
            );
            setTeamData(team);
  ...
  const handleExportReport = async () => {
    ...
    try {
      const reportTypeMap: {
        [key: string]: "overview" | "flow" | "cycle" | "wip" | "team";
      } = {
        overview: "overview",
        flow: "flow",
        cycle: "cycle",
      ...
      link.href = url;
      link.download = `kanban-${currentReportType}-report-${
        ...
        <div className="flex items-center justify-between">
          <div className="flex-1">
            ...
            {trend && trendValue !== undefined && (
              <div
                className={`flex items-center space-x-1 text-sm mt-1 ${getTrendColor(
                  trend
                )}`}
              >
                {getTrendIcon(trend)}
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Flow metrics and team performance insights
          </p>
        </div>
            ...
            <>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                <MetricCard
                  title="Total Tasks"
                  value={
                    overviewData?.metrics.total_tasks ?? metrics.totalTasks
                  }
                  icon={Layers}
                ...
                <MetricCard
                  ...
                  value={
                    overviewData?.metrics.completion_rate ??
                    metrics.completionRate
                  }
                  unit="%"
                  icon={CheckCircle}
                  description={`${
                    overviewData?.metrics.completed_tasks ??
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  ...
                  <CardContent>
                    <div className="space-y-4">
                      <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center space-x-2">
                          <span className="text-sm font-medium">
                            {overviewData?.task_distribution_by_status.todo ??
                              metrics.todoTasks}
                          </span>
                          <Badge variant="outline" className="text-xs">
                            {(overviewData?.metrics.total_tasks ??
                              metrics.totalTasks) > 0
                              ? Math.round(
                                  ((overviewData?.task_distribution_by_status
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center space-x-2">
                          <span className="text-sm font-medium">
                            {overviewData?.task_distribution_by_status.done ??
                              metrics.completedTasks}
                          </span>
                          <Badge variant="outline" className="text-xs">
                            {overviewData?.metrics.completion_rate ??
                      ...
                      {(overviewData?.task_distribution_by_status.blocked ??
                            ...
                            <div className="flex items-center space-x-2">
                              ...
                              >
                                {(overviewData?.metrics.total_tasks ??
                                  ...
                                  ? Math.round(
                                      ((overviewData
                                        ?.task_distribution_by_status.blocked ??
                ...
                <Card>
                  ...
                  <CardContent>
                    <div className="space-y-4">
                      {Object.entries(
                        overviewData?.task_distribution_by_type ??
                          metrics.tasksByType
                      ).map(([type, count]) => (
                        <div key={type}>
              ...
              <Card>
                ...
                <CardContent>
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    {Object.entries(
                      overviewData?.task_distribution_by_priority ??
            ...
            <>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <MetricCard
                  ...
                  value={
                    flowData?.metrics?.flow_efficiency ??
                    flowData?.flow_efficiency ??
                ...
                <CardContent>
                  <div className="space-y-4">
                    ...
                    >
                      ...
                      >
                        {flowData?.efficiency_analysis?.status === "excellent"
                          ? "Excellent Flow"
                          : flowData?.efficiency_analysis?.status === "good"
                          ? "Good Flow"
                          : flowData?.efficiency_analysis?.status ===
                            "needs_improvement"
                          ? "Flow Needs Improvement"
                          : flowData?.insights?.completion_status ===
                            "excellent"
                          ? "Excellent Flow"
                          : flowData?.insights?.completion_status === "good"
                          ? "Good Flow"
                          : flowData?.insights?.completion_status ===
                            "needs_improvement"
                          ? "Flow Needs Improvement"
                      ...
                      >
                        {flowData?.efficiency_analysis?.message ??
                      ...
                      <div>
                        ...
                        <ul className="space-y-1 text-muted-foreground">
                          {flowData?.key_insights ? (
                            <>
                              <li>
                                • {flowData.key_insights.tasks_completed} tasks
                                completed
                              </li>
                              <li>
                                •{" "}
                                {flowData.key_insights.tasks_finished_last_week}{" "}
                              ...
                              <li>
                                ...
                                {
                                  flowData.key_insights
                                    .tasks_currently_in_progress
                                }{" "}
                                tasks currently in progress
                              </li>
                              <li>
                                •{" "}
                                {flowData.key_insights.overall_flow_efficiency?.toFixed(
                                  1
                                )}
                              ...
                              <>
                                <li>
                                  ...
                                  {(flowData?.metrics?.wip_count ??
                                    ...
                                    : "WIP is at a healthy level"}
                                </li>
                                <li>
                                  •{" "}
                                  {metrics.blockedTasks > 0
                                    ? `Address ${metrics.blockedTasks} blocked tasks`
                                    : "No blocked tasks"}
                                </li>
            ...
            <>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <MetricCard
                  ...
                  value={
                    cycleTimeData?.metrics?.average_cycle_time ??
                    cycleTimeData?.average_cycle_time ??
                ...
                <MetricCard
                  ...
                  value={
                    cycleTimeData?.metrics?.average_lead_time ??
                    cycleTimeData?.average_lead_time ??
                ...
                <MetricCard
                  ...
                  value={
                    cycleTimeData?.metrics?.time_efficiency ??
                    cycleTimeData?.time_efficiency ??
                    (metrics.averageLeadTime > 0
                      ? Math.round(
                          (metrics.averageCycleTime / metrics.averageLeadTime) *
                            100
                        )
                      : 0)
                  }
              ...
              <Card>
                <CardHeader>
                  <CardTitle className="text-lg">
                    Cycle Time vs Lead Time Analysis
                  </CardTitle>
                </CardHeader>
                <CardContent>
                  <div className="space-y-4">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div>
                        ...
                        <div className="text-3xl font-bold text-[#007BFF] mb-2">
                          {cycleTimeData?.average_cycle_time ??
                            metrics.averageCycleTime}{" "}
                          days
                        </div>
                        <p className="text-xs text-muted-foreground">
                          {cycleTimeData?.cycle_time_status === "excellent"
                            ? "Excellent"
                            : cycleTimeData?.cycle_time_status === "good"
                            ? "Good"
                            : cycleTimeData?.cycle_time_status ===
                              "needs_improvement"
                            ? "Needs improvement"
                            : (cycleTimeData?.average_cycle_time ??
                      ...
                      <div>
                        ...
                        <div className="text-3xl font-bold text-[#28A745] mb-2">
                          {cycleTimeData?.average_lead_time ??
                            metrics.averageLeadTime}{" "}
                          days
                        </div>
                        <p className="text-xs text-muted-foreground">
                          {cycleTimeData?.lead_time_status === "excellent"
                            ? "Excellent"
                            : cycleTimeData?.lead_time_status === "good"
                            ? "Good"
                            : cycleTimeData?.lead_time_status ===
                              "needs_improvement"
                            ? "Needs improvement"
                            : (cycleTimeData?.average_lead_time ??
                                metrics.averageLeadTime) <= 5
                            ? "Excellent"
                            : (cycleTimeData?.average_lead_time ??
                                metrics.averageLeadTime) <= 14
                            ? "Good"
                            : "Needs improvement"}
                        </p>
                      </div>
                    ...
                    <div className="pt-4 border-t">
                      ...
                      <div className="space-y-2 text-sm text-muted-foreground">
                        <p>
                          • <strong>Cycle Time:</strong> Measures active work
                          time. Lower is better.
                        </p>
                        <p>
                          • <strong>Lead Time:</strong> Measures total time
                          including wait time. Should be close to cycle time.
                        </p>
                        <p>
                          • <strong>Gap Analysis:</strong> Large gap indicates
                          tasks waiting before work starts.
                        </p>
                      </div>
                    </div>
                  </div>
            ...
            <>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <MetricCard
                  title="Aging WIP"
                  value={
                    wipData?.metrics?.aging_wip ??
                    wipData?.aging_wip_count ??
                    metrics.agingWIP.filter((t) => t.isAging).length
                  }
                  icon={AlertCircle}
              ...
              <Card>
                ...
                <CardContent>
                  {(
                    wipData?.work_in_progress_details ??
                    wipData?.wip_tasks ??
                    metrics.agingWIP
                  ).length === 0 ? (
                    ...
                    {(wipData?.aging_wip_count ??
                      ...
                      <div className="p-3 bg-yellow-50 dark:bg-yellow-950 rounded-lg">
                        <p className="font-medium text-yellow-800 dark:text-yellow-200">
                          ⚠️{" "}
                          {wipData?.aging_wip_count ??
                            metrics.agingWIP.filter((t) => t.isAging)
                              .length}{" "}
                          tasks have been in progress for over 7 days
                        </p>
                        <p className="text-yellow-700 dark:text-yellow-300 text-xs mt-1">
                          Review these tasks for blockers or consider breaking
                          them down into smaller items.
                        </p>
                      </div>
                    )}
                    <ul className="space-y-1 text-muted-foreground">
                          ...
                          <>
                            <li>
                              ...
                              {(wipData?.metrics?.current_wip ??
                                wipData?.current_wip ??
                                metrics.wipCount) > 10
                                ? "Consider limiting WIP to improve focus"
                                : "WIP is at a healthy level"}
                            </li>
                            <li>
                              • Focus on completing tasks before starting new
                              ones
                            </li>
                            <li>
                              • Review aging tasks weekly to identify blockers
                            </li>
                            <li>
                              • Maintain a balanced distribution across team
                              members
                            </li>
                          </>
                        );
            ...
            <>
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                <MetricCard
                  title="Team Members"
                  value={
                    teamData?.team_metrics?.team_members_count ??
                    teamData?.team_size ??
                ...
                <MetricCard
                  title="Total Tasks"
                  value={
                    teamData?.team_metrics?.total_tasks ??
                ...
                <MetricCard
                  title="Completed"
                  value={
                    teamData?.team_metrics?.completed_tasks ??
                ...
                <MetricCard
                  ...
                  value={
                    teamData?.team_metrics?.average_completion_rate ??
                    teamData?.average_completion_rate ??
                    (metrics.teamPerformance.length > 0
                      ? Math.round(
                          metrics.teamPerformance.reduce(
              ...
              <Card>
                ...
                <CardContent>
                  {(
                    teamData?.individual_performance ??
                          ...
                          <div className="space-y-2">
                            <div className="flex justify-between text-sm">
                              ...
                              <span className="font-medium">
                                {member.total_tasks ?? member.totalTasks}
                              </span>
                            </div>
                            <div className="flex justify-between text-sm">
                              <span className="text-muted-foreground">
                            ...
                            {member.average_cycle_time && (
                              <div className="flex justify-between text-sm">
                                ...
                                <span className="font-medium">
                                  {member.average_cycle_time} days
                                </span>
                              </div>
                            )}
                            <Progress
                              value={
                                member.completion_rate ?? member.completionRate
                              }
                              className="h-2 mt-2"
                            />
                          </div>
